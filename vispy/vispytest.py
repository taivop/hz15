...
class Canvas(app.Canvas):
	def __init__(self):
		...
		self.device = usb.core.find(address=int("0022bdcf4734", 16))  # Find using mac address
		#self.device = usb.core.find(idVendor=0x06c2, idProduct=0x0036)  # Find using USB Vendor/Product
		...
		print(usb.util.get_string(self.device, 256, 1))
		print(usb.util.get_string(self.device, 256, 2))
		print(usb.util.get_string(self.device, 256, 3))
	...
	def on_timer(self, event):
		...
		y[:, :-1] = y[:, 1:]
		y[:, -1] = [normalize(data0), normalize(data1)]
